...
- **الأزرار العائمة**: WhatsApp + اتصال هاتفي (`src/components/floatingButtons.ts`، الرقم والأيقونة مصدّرين كـ `WHATSAPP_NUMBER`/`WHATSAPP_ICON` — استوردهم بدل ما تكررهم). على الديسكتوب والتابلت: دوائر ثابتة فيزيائياً على يمين الشاشة بكل اللغات. **على الموبايل (≤640px) بيتحولوا لشريط سفلي ثابت** بزرّين بعرض متساوي مع نص قصير (`floatingActions.whatsappShort`/`callShort`)، والـ `body` عليه `padding-bottom` حتى ما يغطّي أي محتوى أو الفوتر — الدوائر العائمة كانت تغطي نصوص وأزرار بالموبايل. **معتمد من المستخدم (2026-09-26) — لا ترجّع الدوائر على الموبايل.** بيختفوا لما قائمة الموبايل مفتوحة (`body.nav-open`).
