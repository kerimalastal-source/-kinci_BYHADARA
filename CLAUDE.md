...
- **فيلا مرمرة هيفن = صفحة هبوط (landing page) لإعلانات ممولة** (رابطها للإعلانات: `/ar/projects/marmara-haven-villa` أو بدون `/ar` للإنجليزي، مع UTM) (سوشيال ميديا + جوجل) — حسب المستخدم أهم مشروع للبيع، فأي تعديل عليها لازم يخدم التحويل (conversion). بالـ hero زر واحد صغير "أنا مهتم" (`/contact?project=`) **بس للمشاريع اللي عليها `landing: true`** (حالياً الفيلا) — قرار المستخدم 2026-09-26: الزرّين الكبار (مهتم + واتساب) بالـ hero لكل المشاريع كانوا ثقال على الموبايل وواتساب مكرر مع الزر العائم، فانشالوا من المشاريع العادية (بيضل كرت "مهتم بالمشروع؟" تحت + الأزرار العائمة). لا ترجّع واتساب للـ hero. `src/utils/campaign.ts` بيحفظ `utm_*`/`gclid`/`fbclid` من رابط الإعلان بـ sessionStorage وبيضيفهم كسطر `Source:` بإيميل الاستفسار — روابط الإعلانات لازم تحمل UTM (مثلاً `?utm_source=facebook&utm_campaign=villa`).
- **شغل معلّق على صفحة الفيلا (بانتظار المستخدم)**:
  - **صور الطوابق**: المستخدم رح يبعت صورة لكل طابق (السفلي، الأرضي، الأول، السطح مع منظر البحر) — حالياً صور مؤقتة من المعرض: السفلي = `side-view` (واجهة جانبية، مش من جوّا الطابق)، الأرضي = `living-room`، الأول = `master-bedroom`، السطح = `terrace` + `seaView`. لما توصل: حطها بـ `public/images/projects/marmara-haven-villa/` وبدّل `image` بـ `floorPlan`.
  - **أكواد التتبّع**: Meta Pixel ID وGoogle Ads/Analytics ID — ما انبعتوا لسا؛ لما يوصلوا بينحطوا بـ `index.html` (مع حدث عند إرسال الفورم/ضغط واتساب).
- **السعر (`price` بـ `Project`)**: `"on-request"` بيعرض "السعر: عند الطلب" (شارة تحت الموقع بالـ hero + سطر "السعر" بأهم المعلومات)، وأي قيمة تانية بتنعرض كما هي (`dir="ltr"`). فيلا مرمرة: عند الطلب (قرار المستخدم 2026-09-26). لا تخترع أسعار.
...
- **الموبايل أولوية (طلب صريح من المستخدم 2026-09-26: أغلب الزبائن من الموبايل، بعدين الآيباد)**: أي تعديل لازم يُختبر أولاً على 390px و360px (وآيباد 768/1024) بالعربي والإنجليزي (والروسي للنصوص الطويلة)، مش بس على الديسكتوب. قواعد ثابتة:
  ...
  - **صفحات المشاريع على الموبايل (طلب المستخدم 2026-09-26)**: الوصف الطويل بيفتح مطوي (7 أسطر) مع زر "اقرأ المزيد/عرض أقل" (`initReadMore`، ≤640px بس)؛ الرقم الخامس الوحيد بشريط الأرقام بيمتد بعرض كامل كسطر أفقي بالنص (أيقونة + رقم + تسمية)؛ "مشاريع أخرى" بتصير صف قابل للسحب (scroll-snap) بدل 3 بطاقات طويلة؛ زر الرجوع ≥40px.
  - **مربعات الأرقام (`.stat-tile`) على الموبايل**: حجم الرقم `min(1.75rem, 6.4vw)` + `nowrap` (كان 1.9rem و"60,000+ m²" ينكسر لسطرين — شكوى المستخدم "الخطوط كبيرة")؛ و`statCounter` بيحط الوحدة بعد الرقم (m²) بـ `<span class="stat-unit">` أصغر (0.62em). نص الجسم يضل 16px (أقل من هيك صعب القراءة وبيعمل zoom بالفورم على الآيفون).
  - **العربي: ممنوع `letter-spacing`** (بيفكّك الحروف المتصلة) — `rtl.css` بيصفّره لكل شي تحت `:lang(ar)` (الشعار مستثنى لأنه `lang="en"`)، وبيكبّر التسميات الصغيرة (badge/eyebrow/تسميات الأرقام) لـ 0.85rem.
...
| تفاصيل مشروع | `/projects/:slug` | `projectDetail.ts` | ترتيب الأقسام: hero (شارات، وزر "أنا مهتم" إذا `landing`) ← شريط الأرقام ← جولة بالفيديو (إذا في `video`) ← نظرة عامة (مطوية على الموبايل) ← أبرز المميزات ← الوحدات ← توزيع الطوابق ← المرافق ← الموقع ← معرض الصور؛ وعلى الجنب (وتحت بالموبايل) أهم المعلومات + كرت "مهتم بالمشروع؟"؛ وبالآخر مشاريع أخرى (صف قابل للسحب بالموبايل) |
...
- **`contactForm.ts`**: فورم تواصل احترافي — **الاسم والإيميل والهاتف حقول إلزامية**، فاليديشن بـ regex، الإرسال عبر `mailto:` (ملاحظة production: لا يرسل فعلياً من السيرفر؛ يحتاج backend/service مثل Formspree).
  - **ربط التواصل بالمشروع (طلب المستخدم 2026-09-26)**: زر "أنا مهتم" بكرت صفحة المشروع بيروح على `/contact?project=<slug>` — الفورم بيعرض بطاقة المشروع فوق (صورة + اسم + منطقة، `.inquiry-project`)، وحقل "المشروع الذي تهتم به" (select بكل المشاريع + "استفسار عام") مختار مسبقاً، ورسالة جاهزة قابلة للتعديل. الإيميل: الموضوع `Project Inquiry: <اسم المشروع بالإنجليزي>` وأول الرسالة اسم المشروع + رابطه، وبعد بيانات الزائر سطر `Language:` (لغة الصفحة) وسطر `Source:` (مصدر الإعلان من `campaign.ts`، إذا موجود). وجنبه زر واتساب برسالة فيها اسم المشروع (`projectDetail.whatsappMessage`). أي زر تواصل جديد مرتبط بمشروع → استخدم نفس `?project=`.
