...
- [ ] نقل الصور من Wix لاستضافة/CDN خاص بالموقع الجديد (اعتماد خارجي حالياً). **المتبقي على Wix**: صور مشاريع `lotus-koru-1` و`lotus-koru-2` و`lotus-istanbul` و`marmara-haven-villa`، وأغلفة مقالات المدونة العشرة (`blog.ts`)، وصورتين بـ `home.ts`/`about.ts`. `static.wixstatic.com` **محجوب بسياسة شبكة بيئة Claude** — النقل بيحتاج إما يضيف المستخدم الدومين للـ allowed domains بإعدادات البيئة (وبعدها سكربت ينزّل كل روابط `wixImg()` لـ `public/images/...` ويستبدلها)، أو يبعت الصور/الكتالوجات مباشرة.
