...
- **`floorPlan` (توزيع الطوابق)**: حقل اختياري بالمشروع (`FloorLevel[]`: gross/net لكل طابق)، واسم الطابق ومميزاته بنفس الترتيب بـ `projectsData.<slug>.floorPlan` (`{name, features[]}`) بالقواميس — بيطلع قسم "توزيع الطوابق" بطاقات بعد الوحدات. مستخدم حالياً بفيلا مرمرة هيفن.
