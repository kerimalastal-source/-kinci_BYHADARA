...
- **`unitsAvailable: true`**: مشروع منفّذ (أو قيد الإنشاء) لسا فيه وحدات للبيع — بيظهر شارة "وحدات متاحة" (نقطتها الخضراء بتنبض متل مؤشر Live — `live-pulse` بـ components.css، بطلب المستخدم) على البطاقة والـ hero وسطر "التوفر" بأهم المعلومات. حسب المستخدم: مشاريع الكتالوجات المنفّذة كلها فيها وحدات متاحة للبيع.
...
- **`floorPlan` (توزيع الطوابق)**: حقل اختياري بالمشروع (`FloorLevel[]`: gross/net لكل طابق)، واسم الطابق ومميزاته بنفس الترتيب بـ `projectsData.<slug>.floorPlan` (`{name, features[]}`) بالقواميس — بيطلع قسم "توزيع الطوابق" بطاقات بعد الوحدات. مستخدم حالياً بفيلا مرمرة هيفن. كل طابق ممكن ياخد `image` (صورة بأعلى البطاقة، بتفتح بالـ lightbox) و`seaView: true` (شارة "إطلالة بحرية" على الصورة).
- **فيلا مرمرة هيفن = صفحة هبوط (landing page) لإعلانات ممولة** (سوشيال ميديا + جوجل) — حسب المستخدم أهم مشروع للبيع، فأي تعديل عليها لازم يخدم التحويل (conversion). بالـ hero لكل المشاريع زرّين: "أنا مهتم" (`/contact?project=`) + واتساب. `src/utils/campaign.ts` بيحفظ `utm_*`/`gclid`/`fbclid` من رابط الإعلان بـ sessionStorage وبيضيفهم كسطر `Source:` بإيميل الاستفسار — روابط الإعلانات لازم تحمل UTM (مثلاً `?utm_source=facebook&utm_campaign=villa`).
- **فيديو المشروع (`video` بـ `Project`)**: `{ youtubeId, vertical?, poster }` — قسم "جولة بالفيديو" تحت شريط الأرقام مباشرة (نص + زرّين + الفيديو). بيحمّل صورة `poster` بس، ومشغّل يوتيوب (`youtube-nocookie`) بينحط لما الزائر يضغط تشغيل (سرعة الصفحة مهمة للإعلانات). `vertical: true` لـ Shorts (إطار 9:16). العنوان والنص من `projectsData.<slug>.videoTitle/videoText`. فيلا مرمرة: Short `kvru16nD3a4`. **يوتيوب محجوب بشبكة بيئة Claude** — ما بتقدر تشوف الفيديو أو صورته المصغّرة، فالـ poster صورة من صور المشروع المحلية.
...
- **الأزرار العائمة**: WhatsApp + اتصال هاتفي (`src/components/floatingButtons.ts`، الرقم والأيقونة مصدّرين كـ `WHATSAPP_NUMBER`/`WHATSAPP_ICON` — استوردهم بدل ما تكررهم)، دوائر ثابتة فيزيائياً على يمين الشاشة بكل اللغات وبكل الأحجام (48px على الموبايل). **قرار نهائي من المستخدم (2026-09-26): تضل دوائر عائمة على الموبايل كمان** — جرّبنا تحويلها لشريط سفلي بالموبايل، وبعد ما شاف الدوائر بموبايله قال "خليها زي ما هيا حلوة"، فرجعت. لا تحولها لشريط. زر الاتصال عليه إطار فاتح حتى يبان فوق الفوتر الأخضر، والفوتر بالموبايل تحت سطر الحقوق فيه مسافة إضافية حتى الدوائر ما تغطيه. بيختفوا لما قائمة الموبايل مفتوحة (`body.nav-open`).
- **الموبايل أولوية (طلب صريح من المستخدم 2026-09-26: أغلب الزبائن من الموبايل، بعدين الآيباد)**: أي تعديل لازم يُختبر أولاً على 390px و360px (وآيباد 768/1024) بالعربي والإنجليزي (والروسي للنصوص الطويلة)، مش بس على الديسكتوب. قواعد ثابتة:
  - **قائمة الموبايل/التابلت (≤1400px)**: لوحة بطول الشاشة تحت الهيدر (`top: var(--header-h)`) بروابط كبيرة، "Resources" بينفتح داخلها كقائمة فرعية مش popup، وبآخرها زرّين (تواصل معنا + واتساب، `.main-nav__actions`). فتحها بيقفل تمرير الصفحة (`body.nav-open`) وبيقلب أيقونة القائمة لـ X، وEscape بيسكّرها. ارتفاع الهيدر متغير `--header-h` (104 / 88 تابلت / 68 موبايل) — لا تكتب `top: 104px` ثابت.
  - **الهيدر على الموبايل**: كل الأزرار 40×40 على الأقل؛ ≤400px زر الدخول بيصير أيقونة شخص دائرية (النص العربي "تسجيل الدخول" كان يدفع زر البحث برا الشاشة).
  - **حقول الفورم ≥16px** على ≤1024px (أصغر من هيك iPhone بيعمل zoom للصفحة عند الضغط). `phone-field` لما يصير عمودي لازم `flex: none` على اختيار الدولة (وإلا الـ flex-basis بيصير ارتفاع ~130px).
  - **الخطوط**: `h1`/`h2` بـ `clamp()` سلّم واحد من الموبايل للديسكتوب (base.css) — لا تحط أحجام عناوين ثابتة لكل صفحة. مسافة الأقسام 3rem موبايل / 4.5rem تابلت.
  - **أي نص `<p>` جوّا خلفية غامقة** لازم لون صريح فاتح (القاعدة العامة `p { color: ink-soft }` بتخليه شبه مخفي) — صار فعلياً بكرت "مهتم بالمشروع؟" وسطر الحقوق بالفوتر.
  - **شبكة من عمودين بالتابلت**: البطاقة الوحيدة بآخر صف بتمتد بعرض كامل كبطاقة أفقية (`.project-grid`)، وشريط أرقام المشروع بـ5 أرقام بيصير 5 أعمدة (641–1100px) — ما في خانات فاضية.
  - **البحث**: بيطابق كمان الاسم الإنجليزي للمشروع + المنطقة + المطوّر بكل اللغات، ومن دون حساسية للأحرف التركية (beylikduzu = Beylikdüzü).
...
- **سكشن "إطلاق جديد" بالرئيسية (spotlight)**: يروّج لمشروع New Launch واحد (حالياً `diamond-marin`) — صورة كبيرة + 3 صور مصغرة قابلة للتبديل، 4 أرقام، شارة "عدد محدود من الوحدات" (بطلب صريح من المستخدم)، وزرّين: واتساب برسالة جاهزة فيها اسم المشروع (`home.spotlightWhatsappMessage`) و"اعرف المزيد" لصفحة التفاصيل. لتغيير المشروع: عدّل `SPOTLIGHT_SLUG` (والأرقام المعروضة `SPOTLIGHT_STATS`) بأعلى `src/pages/home.ts`. شبكة "مشاريع مميزة" تحته **تستثني** مشروع الـ spotlight وتعرض المشروع اللي بعده بالترتيب — قرار نهائي من المستخدم: ما في تكرار لنفس المشروع بالصفحة الرئيسية. خلفيته بطاقة بيضاء (مش أخضر غامق).
...
|---|---|---|---|
| الرئيسية | `/` | `home.ts` | hero، إحصائيات متحركة، شركاء النجاح، **سكشن "إطلاق جديد" (spotlight)** لمشروع واحد قبل شبكة المشاريع، مشاريع مميزة، خدمات، لماذا حضارة، CTA للجنسية |
...
- **`contactForm.ts`**: فورم تواصل احترافي — **الاسم والإيميل والهاتف حقول إلزامية**، فاليديشن بـ regex، الإرسال عبر `mailto:` (ملاحظة production: لا يرسل فعلياً من السيرفر؛ يحتاج backend/service مثل Formspree).
  - **ربط التواصل بالمشروع (طلب المستخدم 2026-09-26)**: زر "أنا مهتم" بكرت صفحة المشروع بيروح على `/contact?project=<slug>` — الفورم بيعرض بطاقة المشروع فوق (صورة + اسم + منطقة، `.inquiry-project`)، وحقل "المشروع الذي تهتم به" (select بكل المشاريع + "استفسار عام") مختار مسبقاً، ورسالة جاهزة قابلة للتعديل. الإيميل: الموضوع `Project Inquiry: <اسم المشروع بالإنجليزي>` وأول الرسالة اسم المشروع + رابطه + لغة الزائر. وجنبه زر واتساب برسالة فيها اسم المشروع (`projectDetail.whatsappMessage`). أي زر تواصل جديد مرتبط بمشروع → استخدم نفس `?project=`.
